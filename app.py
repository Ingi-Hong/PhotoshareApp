...
def register_user():
	...
	if test:
		print(cursor.execute("INSERT INTO users (email, password, is_registered,first_name, last_name, hometown) VALUES ('{0}', '{1}', 1, '{2}','{3}', '{4}')".format(email, password, first_name, last_name, hometown)))
		conn.commit()
		#log user in
		user = User()
		user.id = email
